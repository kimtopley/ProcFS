...
Now you need to make some changes to the kernel source. Note that if you later download a new version of the kernel sources, you'll need to change the **XNU_SOURCE_CODE_DIR** setting in Xcode and make all of these changes again. Bear in mind also that it's not guaranteed that the currect *procfs* source will work with future versions of the operating system source. If Apple makes changes that break *procfs*, I'll endeavor to fix the problem as soon as I find out about it.
...
First, add the three lines bracketted by `PROCFS` below after the section dealing with `MOCKFS`:
